...
impl Astro {
    const TEN_POW_EIGHT: f64 = 10_i32.pow(8) as f64;
    ...
    fn new(julian_day: f64) -> Self {
        ...
        let jc = j / 36525.;
        let jm = jc / 10.;
        let mut jms = Self::pow_series(jm, 5);
        jms.insert(0, 1.);
        ...
        let bs = [&Self::B0[..], &Self::B1];
        let rs = [&Self::R0[..], &Self::R1, &Self::R2, &Self::R3, &Self::R4];

        let lsum = Self::calc_sum(&ls, &jms);
        let bsum = Self::calc_sum(&bs, &jms);
        let rsum = Self::calc_sum(&rs, &jms);
        ...
        let (psi, eps) = Self::calc_psi_eps(&xi_arr, jc);
    ...
    fn calc_psi_eps(elems: &[f64; 5], jc: f64) -> (f64, f64) {
        ...
        Self::SIN_COEFFICIENT.iter().enumerate().fold(
            (0., 0.),
            |acc, (idx, (sc0, sc1, sc2, sc3, sc4))| {
                ...
                let psi = acc.0 + Self::PE[idx].0 + jc * Self::PE[idx].1 * xi_sum_rads.sin();
                // Nutation in obliquity
                let eps = acc.1 + Self::PE[idx].2 + jc * Self::PE[idx].3 * xi_sum_rads.cos();
                (psi, eps)
            },
        )
    }
...
impl TopAstroDay {
    ...
    fn from_ad(astro_day: AstroDay, coords: Coordinates) -> Self {
        ...
        for astro in astro_day.astros.iter() {
            ...
            let p_sin_phi = b_a * u.sin() + elev / Self::EARTH_RADIUS * lat_rads.sin();
            let p_cos_phi = u.cos() + elev / Self::EARTH_RADIUS * lat_rads.cos();
